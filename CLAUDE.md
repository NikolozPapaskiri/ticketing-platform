...
dotnet test                                                        # 79 tests (58 unit + 21 integration; integration needs Docker)
```
...
- **Now — Phase 4 (resilience, Redis, CQRS) then Phase 5 (concurrency + messaging), in progress:**
  1. Payment provider: `IPaymentGateway` port, typed `HttpClient` + `Microsoft.Extensions.Http.Resilience` (retry/backoff/circuit breaker/timeout), idempotency key per charge; failure paths tested with WireMock.Net.
  2. Redis cache-aside on the event graph (`ICacheService` port; **per-tenant cache keys** to prevent cross-tenant leaks; TTL + jitter; invalidation on transitions/ticket-type changes; documented staleness tradeoff for holds).
  3. CQRS-lite read model for browse (denormalized table, transactional sync now; becomes outbox-driven in Phase 5).
  4. Phase 5: oversell prevention as a pluggable `IReservationStrategy` — **all three implemented** (optimistic xmin + retry [DEFAULT], pessimistic `FOR UPDATE`, Redis atomic decrement), selected via `Reservation:Strategy` config; parallel-buyers integration test proving exactly-N sales.
  5. Phase 5: Orders + booking saga (hold → charge → confirm; compensation on failure), **transactional outbox** + RabbitMQ publisher (background dispatcher) + idempotent consumer (dedupe table), hold-expiry background service. Tag `v2-eventdriven`.
  6. Deferred from Phase 5 (documented): ticket PDF/object storage and SignalR live availability move to the Phase 6/7 window.
