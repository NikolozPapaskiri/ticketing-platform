...
- **Now — Phase 3: Authentication & Authorization (in-repo, weeks 6–7):**
  1. Identity user store (three principals: Customer, OrganizerStaff w/ tenant, PlatformAdmin) + register/login endpoints; PBKDF2 via Identity defaults.
  2. JWT bearer issuance (user id, role, `tenant_id` claim) + validation; short-lived access + revocable refresh tokens (rotation).
  3. **Replace `X-Tenant-Id` header trust with the tenant claim** in `TenantResolutionMiddleware` — clients can no longer choose their tenant.
  4. Authorization: roles → policies (`CanManageEvents`, `CanAdministerPlatform`) → resource-based handlers (staff only within their tenant).
  5. Integration tests for the authz matrix (401 anonymous, 403 wrong role, 404 cross-tenant, 200 correct principal). Secrets via user-secrets, never committed.
