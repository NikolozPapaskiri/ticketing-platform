...
public class EventLifecycleTests
{
    ...
    public async Task FullFlow_CreateEvent_AddTicketType_GetGraph()
    {
        ...
        var ttResponse = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/ticket-types",
            new { name = "GA", price = 49.90m, currency = "USD", totalQuantity = 100 });
        Assert.Equal(HttpStatusCode.OK, ttResponse.StatusCode);

        var getResponse = await _client.GetAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}");
    ...
    public async Task Transitions_FollowStateMachine_IllegalMovesReturn409()
    {
        ...
        // Draft -> OnSale
        var publish = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/publish");
        Assert.Equal(HttpStatusCode.NoContent, publish.StatusCode);

        // OnSale -> OnSale is illegal
        var republish = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/publish");
        ...
        var close = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/close");
        Assert.Equal(HttpStatusCode.NoContent, close.StatusCode);
        var reclose = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/close");
    ...
    public async Task Transition_UnknownEvent_Returns404()
    {
        ...
        var response = await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{Guid.NewGuid()}/publish");
    ...
    public async Task List_Paginates_WithStableTotals()
    {
        var tenant = await _client.CreateTenantAsync();
        for (var i = 0; i < 3; i++)
            await _client.CreateEventAsync(tenant.Id, $"Event {i}");

        var response = await _client.GetAsTenantAsync(tenant.Id, "/api/v1/events?page=1&pageSize=2");
    ...
    {
        var tenant = await _client.CreateTenantAsync();
        var ev = await _client.CreateEventAsync(tenant.Id, "Published one");
        await _client.CreateEventAsync(tenant.Id, "Draft one");
        await _client.PostAsTenantAsync(tenant.Id, $"/api/v1/events/{ev.Id}/publish");

        var response = await _client.GetAsTenantAsync(tenant.Id, "/api/v1/events?status=OnSale");
        var page = await response.Content.ReadFromJsonAsync<PageDto<EventListItemDto>>(ApiClientExtensions.Json);
    ...
    public async Task List_PageZero_Returns400()
    {
        ...
        var response = await _client.GetAsTenantAsync(tenant.Id, "/api/v1/events?page=0");
    ...
    public async Task CreateEvent_InvalidBody_Returns400WithFieldErrors()
    {
        ...
        var response = await _client.PostAsTenantAsync(tenant.Id, "/api/v1/events",
